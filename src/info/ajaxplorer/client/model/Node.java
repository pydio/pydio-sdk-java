...
public class Node implements Serializable {
	...
	public static String MAX_NODES = "max_nodes";
	public static String NODE_HAS_CHILDREN = "ajxp_has_children";
	...
	// , columnDefinition="INTEGER REFERENCES `node`(`id`) ON DELETE CASCADE")
	Node parent;
	...
	// temporar properties collection, used for recurse ls remote
	// ls remote will not call dao create properties until it is needed
	private List<Property> localProperties = new ArrayList<Property>();
	...
	public void initFromXmlNode(org.w3c.dom.Node xmlNode) {
		initFromXmlNode(xmlNode, false);
	}

	public void initFromXmlNode(org.w3c.dom.Node xmlNode, boolean tempProp) {
		if (this.resourceType.equals(NODE_TYPE_REPOSITORY)) {
			this.addProperty("icon", "repository.png", tempProp);
			...
			for (int k = 0; k < children.getLength(); k++) {
				 ...
				 * else
				 * if(children.item(k).getNodeName().equals("client_settings")){
				 * this.addProperty("icon",
				 * children.item(k).getAttributes().getNamedItem
				 * ("icon").getNodeValue()); }
				 */
			}
		}
		...
		for (int i = 0; i < map.getLength(); i++) {
			...
			if (NODE_HAS_CHILDREN.equals(name)) {
				try {
					hasChildren = Boolean.parseBoolean(value);
				} catch (Exception e) {
			...
			if (this.resourceType.equals(NODE_TYPE_REPOSITORY)) {
				if (name.equals("id")) {
					this.addProperty("repository_id", value, tempProp);
				} else if (name.equals("repositorySlug")) {
					this.addProperty("slug", value, tempProp);
				} else {
					this.addProperty(name, value, tempProp);
				}
			} else {
				if (name.equals("text")) {
					this.label = value;
		...
		if (getPropertyValue("ajxp_mime") != null
				&& getPropertyValue("ajxp_mime").equalsIgnoreCase(
						"ajxp_browsable_archive")) {
			this.leaf = false;
		}
	...
	public void addProperty(String name, String value, boolean tempProperty) {
	...
	public synchronized void setProperty(String name, String value,
			RuntimeExceptionDao<Property, Integer> dao) {
		if (properties == null)
			return;
		boolean found = false;
		try {
	...
	public synchronized void setProperty(String name, String value,
			Dao<Property, Integer> dao) throws SQLException {
		if (properties == null)
			return;
		boolean found = false;
		try {
	...
	public int deleteProperty(String name,
			RuntimeExceptionDao<Property, Integer> propDao) {
		if (properties == null)
			return 0;
		int count = 0;
		try {
	...
	public String toString() {
		...
		if (parent != null)
			bf.append("Parent Node Id: " + parent.id + "\n");
	...
	public String getUuidPath() {
		return uuid.toString() + (getExtension() != "" ? "." : "")
				+ getExtension();
	}
	...
	public void copyLocalProperties() {
		this.properties.addAll(localProperties);
	}
}
